<?php 
	...
	class Tienda extends Controllers{
		...
		public function tienda()
		{
			...
			//dep($data['categorias']);
			$data['color'] = $this->getCatColor();
			$data['presentacion'] = $this->getCatPresentacion();
			
			if ($_POST) {
				$categoria = $_POST['categoria'];
				$color = $_POST['catColor'];
				$presentacion = $_POST['catPresen'];

				$data['productosFiltrar'] = $this->getProductosPagebusqueda($categoria, $color, $presentacion);
			}else{
				$data['productosFiltrar'] = $this->getProductosPagebusqueda(0, 0, 0);
			}
				...
				if(is_numeric($idproducto) and is_numeric($cantidad)){
					...
					if(!empty($arrInfoProducto)){
						...
						$arrProducto = array('idproducto' => $idproducto,
											...
											'precioDesc' => $arrInfoProducto['precio_desc'],
				...
				if(is_numeric($idproducto) and ($option == 1 or $option == 2)){
					...
					foreach ($_SESSION['arrCarrito'] as $pro) {
						...
						if($pro['precioDesc'] > 0){
							...
							$subtotal += $pro['cantidad'] * $pro['precioDesc'];
				...
				if(is_numeric($idproducto) and $cantidad > 0){
					...
					for ($p=0; $p < count($arrCarrito); $p++) { 
						...
						if($arrCarrito[$p]['idproducto'] == $idproducto){
							...
							if($arrCarrito[$p]['precioDesc'] > 0){
								
								$totalProducto = $arrCarrito[$p]['precioDesc'] * $cantidad;
								break;
							}else{
