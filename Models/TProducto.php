...
trait TProducto{
	...
	private $intCategoriaColor;
	private $intCategoriaPre;
	...
	public function getProductosT(){
		...
		$sql = "SELECT p.idproducto,
				...
				WHERE p.status = 1 ORDER BY p.idproducto DESC LIMIT ".CANTPORDHOME;
	...
	public function getProductosPage($desde, $porpagina){
		...
		public function getProductosPagebusqueda($categoria, $color, $presentacion){

		$this->intCategoria = $categoria;
		$this->intCategoriaColor = $color;
		$this->intCategoriaPre = $presentacion;
		...
		$sql = "SELECT p.idproducto,
				...
				INNER JOIN categoria c
				ON p.categoriaid = c.idcategoria
				WHERE p.categoriaid = '{$this->intCategoria}'AND p.colorid = '{$this->intCategoriaColor}' AND p.presentacionid = '{$this->intCategoriaPre}' AND p.status = 1";
				$request = $this->con->select_all($sql);
				if(count($request) > 0){
					for ($c=0; $c < count($request) ; $c++) { 
						$intIdProducto = $request[$c]['idproducto'];
